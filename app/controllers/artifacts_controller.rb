class ArtifactsController < ApplicationController
  ...
  def index
    @artifacts = Artifact.where(book_id: @book.id).joins(:artifact_type).joins(:character).order('characters.last_name,artifact_types.name, artifacts.name')
  end
  ...
  private
    ...
    def artifact_params
      params.require(:artifact).permit(:name, :character_id, :book_id, :artifact_type_id, :summary)
    end
end
