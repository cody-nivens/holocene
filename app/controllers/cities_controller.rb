class CitiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @city.save
        format.html { redirect_back_or_default(@city, notice: 'City was successfully created.') }
        format.json { render :show, status: :created, location: @city }
      else
  ...
  def update
    respond_to do |format|
      if @city.update(city_params)
        format.html { redirect_back_or_default(@city, notice: 'City was successfully updated.') }
  ...
  def destroy
    @city.destroy
    respond_to do |format|
      format.html { redirect_back_or_default(cities_url, notice: 'City was successfully destroyed.') }
  ...
  def city_params
    params.require(:city).permit(:name, :name_ascii, :lat, :lng, :country, :iso2, :iso3, :admin_name, :capital,
                                 :population, :simple_map_id, :tour_id)
  end
end
