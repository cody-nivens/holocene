class CharacterValue < ApplicationRecord
  ...
  def self.stats_a(characters, category, character_attribute, _character_attribute_2 = nil)
    stats = {}
    stats[category] = {}
    ...
    distinct_values.each do |value|
      ...
      first_count = characters.joins(:character_values).where(
        "character_values.character_attribute_id": character_attribute.id, "character_values.value": value
      )
  ...
  def self.stats_2(characters, category, character_attribute, character_attribute_2 = nil)
    ...
    distinct_values.each do |value|
      ...
      distinct_values_2.each do |value_2|
        abc = characters.where("#{character_attribute_2} = ?", value_2).joins(:character_values).where(
          'character_values.character_attribute_id = ? and value = ?', character_attribute.id, value
        )
