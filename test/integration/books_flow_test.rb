...
class BooksFlowTest < ActionDispatch::IntegrationTest
  ...
  test "creating a nonfictional Book flow" do
    ...
    assert_difference('Book.count') do
        post books_url, params: { book: { body: "A dark and stormy night", name: "Jump the Jam Juice", user_id: @user.id} }
    end
    ...
    assert_select "h2>a", "Jump the Jam Juice"
    ...
    assert_difference('Chapter.count', 1) do
      post polymorphic_url([book, :chapters]), params: { chapter: { body: "Waiting for the Great Pumpkin, Linus sighed.[[slug55]]", name: "The Great Pumpkin Patch", position: 1, scripted_type: 'Book', scripted_id: Book.last.id } }
    end
    ...
    assert_difference('Footnote.count') do
      post chapter_footnote_create_url(:chapter_id => chapter.id), params: { footnote: { body: "Peanuts", noted_id: chapter.id, noted_type: "Chapter", slug: 'newslug' } }
    end
